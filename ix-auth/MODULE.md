...
- 통과 조건이 **게이트웨이 세션의 superadmin·admin** 이다. 그 외에는 403 이고, 애초에 링크를 응답에 싣지 않는다.
- 콘솔은 **자체 로그인을 유지**한다. 프록시가 게이트웨이 세션으로 신원 서버 토큰을 만들어 주지 않으므로, 게이트웨이 세션만 훔쳐도 사용자 관리는 못 한다(이중 방어).
