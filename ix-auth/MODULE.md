...
| 게이트웨이가 `/admin-ui` 를 프록시 | 비권고. 서비스 키 경로와 관리 콘솔 경로가 한 오리진에 섞인다 |

현재 로컬 검증에서는 게이트웨이와 같은 호스트의 별도 포트로만 띄우고 외부에 열지 않는다. 운영 결정은 배포 시점에 내린다.
...
현재 값은 원본 그대로 `0.1.0-SNAPSHOT` 이다 (델타 없음).
