...
--
-- mariadb/ 를 그대로 쓸 수 없는 이유 세 가지. 각 지점에 주석을 달아 뒀다.
--   ① JSON 컬럼의 리터럴 DEFAULT 금지 (ERROR 1101) → 괄호식 DEFAULT ('{}')
--   ② GROUPS 는 8.0.2+ 예약어 (ERROR 1064) → 처음부터 ixauth_groups 로 만든다
--   ③ UUID 타입이 없다 → Hibernate MySQLDialect 가 쓰는 BINARY(16)
--
-- MySQL 로 새로 설치하는 경우만 이 폴더를 탄다 (Flyway {vendor} = jdbc:mysql://).
-- 기존 MariaDB 설치본은 mariadb/ 를 계속 쓰므로 이 파일의 존재가 그쪽을 건드리지 않는다.
-- IX-Auth baseline
...
CREATE TABLE users (
    ...
    -- MySQL 은 PostgreSQL 의 함수형 인덱스(lower(email))를 지원하지 않는다.
    ...
    -- MySQL 8 은 JSON/TEXT/BLOB 의 리터럴 DEFAULT 를 금지한다(ERROR 1101).
    -- 8.0.13+ 의 괄호식 기본값으로 같은 결과를 낸다 (MariaDB 원본: DEFAULT '{}')
...
-- GROUPS 는 MySQL 8.0.2+ 예약어(윈도우 함수 절)라 인용 없이는 CREATE 가 ERROR 1064 다.
-- 백틱으로 감싸도 Hibernate 가 런타임 SELECT 에서 인용 없이 내보내므로 이름 자체를 바꾼다.
-- MariaDB·PostgreSQL 은 기존 설치본이 있어 V12 가 rename 하지만, MySQL 은 신규뿐이라
-- 처음부터 최종 이름으로 만든다 (mysql/V12 는 그래서 할 일이 없다)
CREATE TABLE ixauth_groups (
    id          BIGINT       AUTO_INCREMENT PRIMARY KEY,
    code        VARCHAR(50)  NOT NULL UNIQUE,
    name        VARCHAR(100) NOT NULL,
    description VARCHAR(500),
...
CREATE TABLE user_groups (
    user_id   BIGINT NOT NULL REFERENCES users(id)  ON DELETE CASCADE,
    group_id  BIGINT NOT NULL REFERENCES ixauth_groups(id) ON DELETE CASCADE,
...
-- MySQL 에는 PostgreSQL 의 연산자 클래스(varchar_pattern_ops) 개념이 없다 — 기본 VARCHAR 인덱스가
...
CREATE TABLE sessions (
    -- MySQL 8 에는 UUID 타입이 없다. Hibernate MySQLDialect 가 java.util.UUID 를
    -- binary(16) 으로 매핑하므로 그대로 맞춘다 — CHAR(36) 으로 두면 validate 가
    -- wrong column type 으로 부팅을 거부한다 (MariaDB 원본: UUID)
...
CREATE TABLE audit_logs (
    ...
    ip         VARCHAR(45),
    user_agent VARCHAR(500),
    -- 위 users.attributes 와 같은 이유로 괄호식 기본값 (ERROR 1101 회피)
