...
SELECT 'SUPERADMIN', '최고 관리자', '전 기능 + 타인 세션 쓰기 + 운영 관리 권한', TRUE FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM roles WHERE code = 'SUPERADMIN');

INSERT INTO roles (code, name, description, is_system)
SELECT 'MODERATOR', '중간 관리자', '타인 세션 제안 + 승인·질의 처리', TRUE FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM roles WHERE code = 'MODERATOR');

INSERT INTO roles (code, name, description, is_system)
SELECT 'MEMBER', '구성원', '기본 역할. 본인 세션만 다룬다', TRUE FROM DUAL
