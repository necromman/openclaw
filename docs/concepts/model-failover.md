...
The embedded runtime's existing session setting `retry.provider.maxRetries` overrides its non-rate-limit retry budget. It is not an `openclaw.json` key and does not change a native harness's internal request retries. Native harnesses may finish their own request retries before OpenClaw begins continuation recovery; the reply runner does not add another whole-turn replay loop. See [Retry policy](/concepts/retry) for pacing and exclusions.
