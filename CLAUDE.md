...
| [chris-local/AEO-GEO.md](chris-local/AEO-GEO.md) | AGEO 에이전트(AEO/GEO 전문) `ageo` 설계·부서 노출 제한·스킬·배포. 타겟 라포르몰 컨텍스트는 3-2 절과 시드 `workspace-seed/ageo/COMPANY.md`, 조사 원문은 [AGEO-RESEARCH-2026-09-14.md](chris-local/AGEO-RESEARCH-2026-09-14.md) |
...
- 머지 완료(D~K 단계 전부, 마지막 머지 커밋 `fdc3b837012`): 브랜딩 제거, 스타일 공통화, 문서 미리보기(PDF·Office), 브라우저 도구, IX-Auth 로그인(= 기기 자동 인증)·로그아웃, 역할 5단계(시스템 관리자·관리자·임원·중재자·직원), 부서 경계, 초대·가입, 앱 내 사용자 관리, 읽기 전용 프로필·NAS 마운트 배선, 사람 귀속 감사 원장, NAS 문서 마크다운 색인(`openclaw knowledge sync`), 관리자 WS 권한 결함 수정, IX-Auth 콘솔 SSO, 부서·에이전트·폴더 관리 화면(부서 삭제 포함), 시드 정리 스크립트, xlsx 날짜 서식, 권한 칩 라벨 수정.
- 코드 잔여 없음. 부서 이름 변경은 벤더 API 부재로 게이트웨이 표시 이름만 바뀐다.
- 사용자 결정 대기: 부서 에이전트용 API 키(ChatGPT 구독 codex 경로로는 폴더 경계를 지키며 읽을 수 없음), 감사 보존기간·질문 본문 기록, 시드 계정 삭제, 제품명, IX-Auth 라이선스, SMTP·도메인·TLS, 시작 번들 예산 기준선 갱신 여부. 상세는 DELIVERY-PLAN.md 4절.
