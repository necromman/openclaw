...
export const toolIcons = {
  ...
  />`),
  // Fork brand mark. The key stays `lobster` because it is an identifier that
  // navigation, the command palette and the profile hero all reference; only the
  // artwork is this fork's own neutral geometry (mirrors ui/public/favicon.svg).
  lobster: html`
    <svg viewBox="0 0 120 120" fill="none">
      <defs>
        <linearGradient id="brand-icon-plate" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" style="stop-color: var(--brand-icon-body, #3f4a5a)" />
          <stop offset="100%" style="stop-color: var(--brand-icon-shade, #1b2029)" />
        </linearGradient>
      </defs>
      <rect x="4" y="4" width="112" height="112" rx="5" ry="5" fill="url(#brand-icon-plate)" />
      <path
        d="M80 40 A28 28 0 1 0 80 80"
        fill="none"
        style="stroke: var(--brand-icon-glyph, #e8edf4)"
        stroke-width="11"
        stroke-linecap="round"
      />
      <circle cx="84" cy="60" r="7" style="fill: var(--brand-icon-accent, #8fb4d9)" />
    </svg>
  `,
