...
describe("openclaw-router-outlet", () => {
  ...
  it("replaces accessible loading text with the resolved route", async () => {
    vi.useFakeTimers();
    ...
    expect(loadingState?.textContent).toContain("화면을 불러오는 중입니다.");
    expect(loadingState?.querySelector("openclaw-mascot")).toBeNull();
