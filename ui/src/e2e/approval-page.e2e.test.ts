...
suite.define(() => {
  ...
  it("keeps one canonical winner across conflicting surfaces and terminal reload", async () => {
    ...
    try {
      ...
      expect(await mobile.page.title()).toBe(`Command approval — ${BRAND_NAME}`);
      ...
      expect(await mobile.page.title()).toBe(`Approved here — ${BRAND_NAME}`);
      ...
      expect(await mobile.page.title()).toBe(`Approved — ${BRAND_NAME}`);
      expect(new URL(mobile.page.url()).pathname).toBe(approvalPath(""));
  ...
  it("retains context but disables decisions through a reconnect", async () => {
    ...
    await surface.page
      .getByText(
        `${BRAND_NAME} cannot confirm or record a decision while disconnected. Reconnect to check the current status.`,
        { exact: true },
      )
      .waitFor();
