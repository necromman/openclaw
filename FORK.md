...
> **2026-09-08 사용자 확정: `chris/main` 에서 직접 작업한다.** 단계별 브랜치와 별도 작업 트리를 만들지 않고, 의미 단위로 커밋해 바로 푸시한다. `.claude/settings.json` 의 PreToolUse 훅이 그 명령들을 차단한다. 푸시가 곧 배포다: GitHub Actions 가 이미지를 굽고 진바이오 NAS 의 cron 이 5분 안에 반영하며, 중간 상태가 운영에 반영돼도 된다.
>
> 같은 결정으로 **WSL 검증 왕복을 하지 않는다.** `pnpm check`·`pnpm test`·`pnpm format` 과 로컬 compose 실측 대신, Actions 이미지 빌드 성공과 `https://jinbio.botops.cloud` 브라우저 실측으로 판정한다. 저장소 규칙(700줄 상한·env 이름 래칫·번들 상한)은 CI 가 걸러 준다. 아래 4절의 동기화 절차에 나오는 빌드·검증 명령은 업스트림 태그를 갈아탈 때처럼 손으로 확인이 필요한 경우에만 쓴다.
...
| pull 전에 빌드 산출물만 되돌린다                      | 빌드가 다시 쓰는 추적 파일 때문에 `--ff-only` 가 계속 거절되는 것을 막는다. 되돌린 이름은 로그에 남는다 |
