...
describe("catalog-only provider preparation", () => {
  it.each(["openclaw", "fixture-cli"])(
    "retains trusted installation and the selected %s runtime",
    async (runtimeId) => {
      vi.mocked(prepareAuthChoiceLoadedPluginProvider).mockResolvedValue({
        config: {
          ...
          agents: {
            ...config.agents,
            defaults: { models: { "fixture/test-model": { agentRuntime: { id: runtimeId } } } },
          },
          plugins: {
        ...
        agentModelOverride: "fixture/test-model",
        authProfiles: [],
        persistAuthProfiles,
      });
