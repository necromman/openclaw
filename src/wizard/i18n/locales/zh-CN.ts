...
export const zh_CN = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密码存入 {brand} 配置",
      ...
      tokenStoreProvisioned:
        "已生成 Gateway 令牌并以 {name} 存入 {brand} 密钥存储。配置中只保留引用；可用 `openclaw secrets store list` 查看。",
    ...
    memoryImport: {
      ...
      confirm: "将这些 memory 导入你的 {brand} 工作区？",
    ...
    remote: {
      ...
      plaintextPasswordHint: "直接把密码存入 {brand} 配置",
      plaintextPasswordLabel: "现在输入密码",
      plaintextTokenHint: "直接把令牌存入 {brand} 配置",
    ...
    guided: {
      ...
      laneQuestion: "你想如何开始？",
      laneSecurityLine:
        "{brand} 运行的 AI 智能体拥有这台机器的实际访问权限。安全指南：https://docs.openclaw.ai/gateway/security",
      ...
      custodianIntro: "你好 — 我是 {brand}，负责维护这套系统。我们开始设置吧。",
      ...
      appliedTitle: "推理已就绪",
      complete: "{brand} 已准备就绪。",
      completeWithoutAi: "{brand} 设置已保存。连接 AI 后再打开聊天。",
      detected: "AI 检测完成。",
      ...
      intro: "连接你的 AI",
      invalidConfigDetails: "{brand} 配置 {path} 无效：\n{issues}",
      invalidConfigRepair:
    ...
    security: {
      ...
      confirm: "我理解 {brand} 默认面向个人使用；共享/多用户使用需要加固。继续？",
      hardeningRequired: "如果你不熟悉访问控制和安全加固，请在获得帮助前不要运行 {brand}。",
      learnMore: "了解更多请访问",
      notMultitenant: "默认情况下，{brand} 并非旨在安全地隔离多个用户。",
      personalAgent: "默认情况下，{brand} 是供一位操作员使用的个人 agent。",
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要发送者/所有者号码，以便 {brand} 将你加入允许列表。",
      ...
      separatePhoneLabel: "{brand} 专用独立手机",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "这是 {brand} 用来向 Chat 发送回复的 URL。",
      ...
      publicWebhookUrlScope: "仅公开此 webhook 路由；{brand} 使用它提供短期附件下载。",
    ...
    signal: {
      ...
      multipleEntries: "多个条目请用逗号分隔。",
      nextLinkDevice: '绑定设备：signal-cli link -n "{brand}"',
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "将 Synology NAS Chat 连接到 {brand}，并启用完整 agent 能力。",
    ...
    finalize: {
      ...
      dashboardCopyPaste: "在本机浏览器中复制/粘贴这个 URL 来控制 {brand}。",
      dashboardOpened: "已在浏览器中打开。保留该标签页以控制 {brand}。",
      ...
      optionalApps: "可选应用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 链接控制 {brand}。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已打开；保留该标签页以控制 {brand}。",
      ...
      containerSystemdUnavailable:
        "此容器内没有 systemd 用户服务。{brand} 只会跳过后台服务安装；请以前台方式运行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
