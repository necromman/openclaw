...
export const zh_TW = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密碼存入 {brand} 設定",
      ...
      tokenStoreProvisioned:
        "已產生 Gateway 權杖並以 {name} 存入 {brand} 祕密儲存。設定中只保留參照；可用 `openclaw secrets store list` 檢視。",
    ...
    memoryImport: {
      ...
      confirm: "將這些 memory 匯入你的 {brand} 工作區？",
    ...
    remote: {
      ...
      plaintextPasswordHint: "直接把密碼存入 {brand} 設定",
      plaintextPasswordLabel: "現在輸入密碼",
      plaintextTokenHint: "直接把權杖存入 {brand} 設定",
    ...
    guided: {
      ...
      laneQuestion: "你想如何開始？",
      laneSecurityLine:
        "{brand} 執行的 AI 代理程式擁有這台機器的實際存取權限。安全指南：https://docs.openclaw.ai/gateway/security",
      ...
      custodianIntro: "你好 — 我是 {brand}，負責維護這套系統。我們開始設定吧。",
      ...
      appliedTitle: "推理已就緒",
      complete: "{brand} 已準備就緒。",
      completeWithoutAi: "{brand} 設定已儲存。連接 AI 後再開啟聊天。",
      detected: "AI 偵測完成。",
      ...
      intro: "連接你的 AI",
      invalidConfigDetails: "{brand} 設定 {path} 無效：\n{issues}",
      invalidConfigRepair:
    ...
    security: {
      ...
      confirm: "我理解 {brand} 預設面向個人使用；共享/多使用者使用需要加固。繼續？",
      hardeningRequired: "如果你不熟悉存取控制和安全加固，請在獲得協助前不要執行 {brand}。",
      learnMore: "了解更多請參閱",
      notMultitenant: "預設情況下，{brand} 並非旨在安全地隔離多個使用者。",
      personalAgent: "預設情況下，{brand} 是供一位操作者使用的個人 agent。",
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要傳送者/擁有者號碼，以便 {brand} 將你加入允許清單。",
      ...
      separatePhoneLabel: "{brand} 專用獨立手機",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "這是 {brand} 用來向 Chat 傳送回覆的 URL。",
      ...
      publicWebhookUrlScope: "僅公開此 webhook 路由；{brand} 使用它提供短期附件下載。",
    ...
    signal: {
      ...
      multipleEntries: "多個項目請用逗號分隔。",
      nextLinkDevice: '綁定裝置：signal-cli link -n "{brand}"',
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "將 Synology NAS Chat 連接到 {brand}，並啟用完整 agent 能力。",
    ...
    finalize: {
      ...
      dashboardCopyPaste: "在本機瀏覽器中複製/貼上這個 URL 來控制 {brand}。",
      dashboardOpened: "已在瀏覽器中開啟。保留該分頁以控制 {brand}。",
      ...
      optionalApps: "可選應用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 連結控制 {brand}。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已開啟；保留該分頁以控制 {brand}。",
      ...
      containerSystemdUnavailable:
        "此容器內沒有 systemd 使用者服務。{brand} 只會略過背景服務安裝；請以前景方式執行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
