...
### 부서별 폴더 권한 바로 설정 (2026-09-13)

부서 목록의 폴더 권한 버튼으로 해당 부서의 규칙만 편집하는 모달을 연다. 사용자 상세의 폴더 권한 탭에서도 부서 공통 대상을 선택할 수 있다. 두 화면은 기존 폴더 권한 편집기와 RPC를 공유하고 부서 slug를 정규 대상으로 사용한다. 부서 소속 자체가 자료 접근을 자동 허용하지는 않는다.
