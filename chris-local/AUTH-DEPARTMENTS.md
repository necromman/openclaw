...
## 0. 지금 납품에 실제로 켜져 있는 것 (2026-09-08, Q 단계)

**부서 게이트는 꺼져 있고, 세션은 "본인만" 이다.** Q 단계에서 `tools.sessions.visibility` 를 `department` 에서 `self` 로 내렸다(사용자 지시 2026-09-08). 4절이 설명하듯 그 값이 곧 부서 게이트의 스위치이므로, 이 문서가 설명하는 부서 경계 코드는 **지금 도는 구성에서 전부 비활성**이다. 부서(`dept-rnd`·`dept-qa`)·부서 소속·부서 관리 화면은 그대로 살아 있고, 값을 `department` 로 되돌리면 경계도 그대로 돌아온다.

**부서 에이전트도 보류다.** 납품 스택의 에이전트는 `main` 하나이고 어느 부서에도 묶여 있지 않다(사용자 결정 2026-09-08, [DELIVERY-PLAN.md](DELIVERY-PLAN.md) 4절 10번). 그래서 게이트를 꺼도 잃는 것이 거의 없다: 미바인딩 에이전트는 게이트가 켜져 있어도 부서가 있는 사람 전원에게 `open` 이었다(2절 세 번째 줄과 `department-access.ts` 의 `agentAccess`).

| 이 문서에서              | 지금 상태                                                                                                              |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| 2·7절 세션 가시성 게이트 | **꺼져 있다.** 대신 역할 상한이 세션을 가른다(3절 표 아래 "Q 단계" 절)                                                 |
| 7-1절 채팅 첨부 경계     | **켜져 있다.** 소유권 강제의 스위치는 `gateway.auth.mode` 하나뿐이라 가시성 값과 무관하다. 부서 비교 단계만 비활성이다 |
| 9절 부서 운영            | **켜져 있다.** 부서 생성·삭제, 사람 배치, 화면 모두 그대로                                                             |
| 13절 폴더 경계(경로 A)   | **보류.** 코드는 전부 있고 설정 항목만 없다. 되켜는 절차는 [DEPLOY.md](DEPLOY.md) 3.3-1                                |

2·7·13절을 읽을 때는 그것이 "지금 도는 구성" 이 아니라 **"켤 수 있는 구성"** 이라는 것을 염두에 둔다. 실측 기록(13.7·13.9)은 그대로 유효하다.

**부서 에이전트를 다시 켜는 날 함께 해야 할 일**: `tools.sessions.visibility` 를 `department` 로 되돌린다. 그러지 않으면 에이전트를 부서에 묶어도 그 바인딩을 읽는 코드가 통째로 비활성이라 아무것도 막지 않는다. [DEPLOY.md](DEPLOY.md) 3.3-1 의 다섯 단계에 이 한 줄이 앞선다.
...
> 이 표는 역할 축의 설계값이다. **지금 납품 템플릿의 값은 다르다**: Q 단계에서 `moderator` 와
> `member` 의 상한을 `none` 으로 낮췄다(3-1절). 아래 문단들은 상한 자체의 성질을 설명한다.
...
### 3-1. Q 단계 - 게이트를 끈 뒤 세션을 무엇이 가르는가 (2026-09-08)

부서 게이트가 꺼지면 **사람이 보는 세션 목록을 정하는 것은 역할 상한 하나뿐**이다. 그 판정은
`tools.sessions.visibility` 를 읽지 않는다. 근거는 세 곳이다.

- `src/gateway/server-methods/sessions-board-inventory.ts` 의 행 술어는 `prepareSessionSharing`
  (역할 상한)과 `prepareDepartmentGate`(부서) 둘만 곱한다. 가시성 값 자체는 없다.
- `src/gateway/session-sharing.ts` 의 `createProfileSessionEntryFilter` 가 그 상한을 집행한다.
  통과 조건은 `내가 만든 세션이거나 (sessionCap !== "none" 이고 draft 가 아님)` 이다. 즉
  **상한이 `none` 이 아니면 남의 비-draft 세션이 전부 보인다.**
- `src/gateway/server-methods/session-catalog-visibility.ts` 도 같은 축을 쓴다.

그래서 "본인만" 을 실제로 만들려면 값 하나가 아니라 둘을 바꿔야 했다. 납품 템플릿
(`chris-local/ixauth-gateway-config/openclaw.json`)의 지금 값과 그 결과:

| 역할         | `sessions.others`    | 세션 목록에서 보이는 것                                    |
| ------------ | -------------------- | ---------------------------------------------------------- |
| `superadmin` | `write`              | 전부(`operator.admin` 이 있어 필터 자체가 만들어지지 않음) |
| `admin`      | `write`              | 전부. 열고 쓸 수 있다                                      |
| `executive`  | `view`               | 전부. **읽기만** 된다                                      |
| `moderator`  | `none` (Q 에서 낮춤) | 자기가 만든 세션만                                         |
| `member`     | `none` (Q 에서 낮춤) | 자기가 만든 세션만                                         |

`moderator` 를 `suggest` 에서 내린 것은 사용자 지시("직원·중재자는 자기 세션만")를 그대로 따른
것이다. 중재자에게 남는 것은 승인 처리 스코프(`operator.approvals`)이고, 남의 세션에 제안을
다는 기능은 이 배포에서 쓰지 않는다.

`tools.sessions.visibility=self` 가 여전히 하는 일이 하나 있다. **모델의 세션 도구**
(`sessions_list`·`sessions_history`·`sessions_send`)를 현재 세션 하나로 묶는다(4절 마지막 문단).
`department` 였을 때 그 축은 `agent` 와 같은 판정이었으므로 이 변경은 그쪽에서도 좁아지는 쪽이다.
...
> **지금 납품 값은 `self` 다**(Q 단계, 0절). 아래 설명은 `department` 로 되돌렸을 때의 동작이다.
...
> **현재 납품은 `main` 단일이고 이 절의 부서 에이전트는 보류다**(0절). 아래는 켜는 방법과 그때 성립하는 성질이며, 되켜는 순서는 [DEPLOY.md](DEPLOY.md) 3.3-1 이다.
>
...
**설정에서 에이전트를 지울 때는 바인딩도 같이 지운다.** 둘이 다른 곳에 살기 때문에, `agents.entries` 에서 항목만 빼면 상태 DB 에 없는 에이전트를 가리키는 행이 남는다. O 단계에서 `rnd-bot`·`qa-bot` 을 뺄 때 실제로 `--clear` 를 따로 돌렸다. 남은 행이 권한을 넓히지는 않지만(존재하지 않는 에이전트는 목록에 나오지 않는다) 같은 id 로 새 에이전트를 만들면 조용히 옛 부서에 묶인다.
...
그래서 읽기 전용 마운트를 워크스페이스로 삼는 에이전트에는 그 항목에 `skipBootstrap: true` 를 함께 적는다\
([DEPLOY.md](DEPLOY.md) 3.3-1 (가)). `main` 은 쓰기 가능한 워크스페이스를 그대로 쓰고\
부트스트랩 파일도 그대로 받는다.
...
부서 이름을 누르면 구성원 관리 모달이 열린다. 기본 정보, 구성원, 폴더 권한, 비서 연결 탭에서 관리하며 목록의 폴더 권한 버튼은 같은 모달의 해당 탭으로 바로 이동한다. 저장 중에는 닫기를 막고, 미저장 변경이 있으면 버리기 여부를 확인한다. 모바일에서는 화면 전체를 사용한다.

사용자 상세의 폴더 권한 탭에서도 부서 공통 대상을 선택할 수 있다. 두 화면은 기존 폴더 권한 편집기와 RPC를 공유하고 부서 slug를 정규 대상으로 사용한다. 부서 소속 자체가 자료 접근을 자동 허용하지는 않는다.

## 15. 직책 (2026-09-13)

부서와 완전히 대칭인 두 번째 분류다. **부서는 사람이 어디에 속하는가, 직책은 그 사람이 무엇인가**를 말한다. 권한 등급 5단계(`SUPERADMIN`·`ADMIN`·`EXECUTIVE`·`MODERATOR`·`MEMBER`)는 전혀 건드리지 않았다. 직책은 아무 권한도 올리거나 내리지 않고, **폴더 규칙의 대상**으로만 쓰인다.

### 15.1 구현 (IX-Auth 그룹 프리픽스)

| 항목        | 부서                                     | 직책                                |
| ----------- | ---------------------------------------- | ----------------------------------- |
| 그룹 코드   | `dept-<slug>`                            | `title-<slug>`                      |
| 설정 키     | `gateway.auth.ixAuth.departmentGroupPrefix` | `gateway.auth.ixAuth.titleGroupPrefix` |
| 기본값      | `dept-`                                  | `title-`                            |
| 클레임      | `ixauth_groups` (같은 클레임)            | `ixauth_groups` (같은 클레임)       |
| principal   | `departments: string[]`                  | `titles: string[]`                  |
| 투영 테이블 | `departments` · `department_members`     | `titles` · `title_members`          |
| 에이전트 바인딩 | `department_agents` 있음              | **없다**                            |
| 1인당 개수  | 여러 개                                  | 여러 개                             |

두 프리픽스는 같은 그룹 클레임을 서로 다른 접두사로 읽는다. 공통 판독기는 `readIxAuthPrefixedGroupCodes`(`src/auth/ix-auth/ix-auth-claims.ts`) 하나다. 환경 변수는 새로 만들지 않았다.

**에이전트 바인딩 표를 두지 않은 이유**: 세션은 에이전트 하나의 저장소 안에 살기 때문에 에이전트가 이미 데이터 경계다(6.3). 부서는 그 경계를 나누지만 직책은 그 경계를 가로지른다. 직책까지 에이전트에 묶으면 한 에이전트에 주인이 둘 생기고 서로 어긋날 수 있다.

### 15.2 폴더 규칙에서의 우선순위

`src/gateway/folder-access-policy.ts` 의 `SUBJECT_PRECEDENCE` 는 이제 네 단계다.

```text
role(0) < title(1) < department(2) < user(3)
```

규칙은 그대로 **경로 우선, 대상 나중**이다. 폴더와 그 조상들을 깊은 쪽부터 훑다가 이 사람에게 걸리는 규칙이 처음 나오는 층에서 멈추고, 그 층 안에서 위 순서로 더 구체적인 대상이 이긴다.

- 직책이 역할보다 강한 이유: 역할은 배포 전체가 공유하는 등급이고, 직책은 이 회사 안에서 그 사람이 무엇인지를 말한다. 더 구체적인 진술이 뒤에 온다.
- 직책이 부서보다 약한 이유: 부서 규칙이 직책 규칙을 덮을 수 있어야 "팀장은 다 읽되 이 부서 안에서는 막는다" 가 표현된다.
- **직책 두 개를 가진 사람은 둘 중 더 좁은 권한을 받는다.** 부서와 같은 규칙이고 이유도 같다. 분류를 더하는 것은 경계를 넓히는 수단이 아니다.

### 15.3 표면

| 표면                                   | 무엇                                                                      |
| -------------------------------------- | ------------------------------------------------------------------------- |
| `GET/POST/PATCH/DELETE /auth/admin/titles` | 목록·추가·이름 변경·삭제. 시스템 관리자와 관리자 모두 사용             |
| `PUT /auth/admin/users/{id}/titles`    | 한 계정의 직책 집합 교체. 본인 행은 거부, 시스템 관리자 대상은 관리자에게 거부 |
| `GET /auth/me` 의 `user.titles`        | 검증된 토큰에서 읽은 직책 slug 배열                                       |
| `folders.subjects.list` 의 `titles`    | 폴더 규칙 편집기의 "직책별" 탭 목록                                       |
| `/settings/titles`                     | 직책 목록·추가·이름 변경·삭제 화면(설정 내비의 부서 아래)                 |
| 사용자 상세 "역할·부서" 탭             | 직책 다중 선택 체크박스                                                   |
| 폴더 권한 화면 두 곳                   | 폴더별 패널·대상별 패널에 "직책별" 탭 추가                                |

감사 원장 액션 이름은 `title-create`·`title-rename`·`title-delete`·`user-titles` 로 부서와 구분된다.

### 15.4 제약 (부서와 같은 것, 다른 것)

- **이름 변경은 게이트웨이 표시 이름만 바꾼다.** 벤더에 그룹 이름 변경 API 가 없다는 사실은 직책에도 그대로다(12절).
- **아무도 가지지 않은 직책만 삭제한다.** 인원수는 신원 서버의 디렉터리를 먼저 보고, 그 호출이 실패하면 503 으로 거부한다. 로컬 투영만 믿고 지우지 않는다.
- **삭제 순서는 그룹 먼저, 투영 나중이다.** 중간에 실패하면 화면이 이미 아는 고아 상태로 남는다.
- **삭제된 직책을 지정한 폴더 규칙은 지우지 않는다.** 폴더 화면의 고아 규칙 목록(`folders.rules.orphans`)에 `missing-subject` 로 뜬다. 조용히 지우면 누가 닫아 둔 폴더가 다시 열린다.
- **반영 지연은 부서와 같다.** 열려 있는 WS 는 핸드셰이크 시점의 직책을 유지한다. 사용자 직책 변경 경로는 그 사람의 신원 세션을 끊어 즉시 반영시킨다.
- 초대 화면에는 직책 선택이 없다(이번 범위 밖). 가입 후 사용자 상세에서 부여한다.

### 15.5 되돌리기

```text
1. 직책을 쓰는 폴더 규칙을 지운다(폴더 화면 또는 folders.rules.clear)
2. /settings/titles 에서 직책을 삭제한다(신원 서버 그룹과 투영이 함께 사라진다)
3. 표를 정말 지우려면 DROP TABLE title_members, titles
   - 지워도 다음 로그인이 다시 채운다. 그룹이 남아 있으면 직책도 남는다
```
