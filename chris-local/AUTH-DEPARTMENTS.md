...
## 0. 지금 납품에 실제로 켜져 있는 것 (2026-09-08, O 단계)

**부서 에이전트는 보류다.** 납품 스택의 에이전트는 `main` 하나이고 어느 부서에도 묶여 있지 않다(사용자 결정 2026-09-08, [DELIVERY-PLAN.md](DELIVERY-PLAN.md) 4절 10번). 부서(`dept-rnd`·`dept-qa`)와 부서 소속은 그대로 살아 있다.

| 이 문서에서              | 지금 상태                                                                              |
| ------------------------ | -------------------------------------------------------------------------------------- |
| 2·7절 세션 가시성 게이트 | **켜져 있다.** 에이전트가 하나여도 부서가 다른 사람의 세션은 서로 안 보인다            |
| 7-1절 채팅 첨부 경계     | **켜져 있다**                                                                          |
| 9절 부서 운영            | **켜져 있다.** 부서 생성·삭제, 사람 배치, 화면 모두 그대로                             |
| 13절 폴더 경계(경로 A)   | **보류.** 코드는 전부 있고 설정 항목만 없다. 되켜는 절차는 [DEPLOY.md](DEPLOY.md) 3.3-1 |

13절을 읽을 때는 그것이 "지금 도는 구성" 이 아니라 **"켤 수 있는 구성"** 이라는 것을 염두에 둔다. 실측 기록(13.7·13.9)은 그대로 유효하다.
