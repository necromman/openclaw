...
| **rhwp v0.8.2 (채택, 1순위)** | **17/17** | 0    | 표를 마크다운 표로 낸다(병합 셀 포함). 9-5                                                   |
| **내장 리더 (채택, 폴백)**    | **17/17** | 0    | 표 칸까지 전부(구조는 사라진다). 최대 59ms                                                   |

rhwp 는 2026-09-09 사용자 지시로 다시 실측해 **1순위 변환기로 채택**했다. 근거·비교·한계는 9-5 절이다.
...
### 9-5. rhwp 재실측과 채택 (2026-09-09, 사용자 지시)

`rhwp`(github.com/edwardkim/rhwp)는 Rust 로 쓴 HWP/HWPX/HML 파서·레이아웃 엔진 CLI 다. **MIT**, 단일 실행 파일, 한컴오피스·LibreOffice·폰트 어느 것도 필요 없다. 별 3.8천, 최근 커밋 2026-09-08 로 활발하다. 사내 스킬 `hwp-form-fill` 이 이미 이 도구로 양식을 채운다.

**판정: 조건부 적합. 버전을 v0.8.2 에 고정해야 한다.**

| 항목                            | 판정              | 근거(실측)                                                                                          |
| ------------------------------- | ----------------- | --------------------------------------------------------------------------------------------------- |
| 리눅스에서 GUI·한컴 없이 도는가 | 예                | `node:24-bookworm-slim` 컨테이너에서 그대로 실행된다                                                |
| glibc                           | **v0.8.2 까지만** | v0.8.3·0.8.4·0.8.6 은 `GLIBC_2.39` 를 요구해 bookworm(2.36)에서 실행 자체가 안 된다. v0.8.2 는 정상 |
| 라이선스                        | 문제 없음         | MIT. 소스 공개, 재배포 가능                                                                         |
| 이미지 증가                     | **+14.9 MB**      | 레이어 실측. LibreOffice 약 580 MiB 와 비교하면 무시할 수준                                         |
| 런타임 의존성                   | 없음              | `libc`·`libgcc`·`libm` 뿐. 텍스트·표 추출에는 폰트도 필요 없다                                      |
| 무결성                          | 확인              | 배포본 `SHA256SUMS.txt` 대조. Dockerfile 이 같은 해시를 다시 검사한다                               |

#### 무엇이 더 나은가 (같은 표본, 같은 컨테이너)

| 항목                 | 내장 리더                               | rhwp v0.8.2                                                                                                   |
| -------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| 성공률(표본 1, 18개) | 17/17                                   | 17/17                                                                                                         |
| 성공률(표본 2, 52개) | 51/52                                   | 51/52 (실패 1건은 둘 다 0바이트 파일)                                                                         |
| 표 구조              | **사라진다.** 칸마다 문단 하나로 풀린다 | **마크다운 표로 나온다.** 병합 셀(rowSpan/colSpan)·중첩 표 보존. `export-tables --json` 은 격자 좌표까지 준다 |
| 쪽 구분              | 없다                                    | **쪽마다 파일 하나.** 사이드카에 `## p.N` 을 붙일 수 있어 인용이 원본 쪽을 가리킨다                           |
| 결재란·머리말        | 글자만 나온다                           | 결재란이 `rowSpan=2` 표로 그대로 나온다                                                                       |
| 그림 속 글자         | 못 읽는다                               | 못 읽는다(OCR 없음). 그림 파일은 따로 뽑아 준다                                                               |
| 속도                 | 파일당 0~59ms(인프로세스)               | 파일당 5~70ms, 284쪽 문서 669ms. 70개 2코어 2.4초                                                             |
| 실패 사유 구분       | **암호·배포용·HWP 3.0 을 구분한다**     | 대부분 하나의 파싱 오류로 합쳐진다                                                                            |
| 지원 형식            | hwp 5.0 · hwpx                          | hwp 5.0 · hwpx · 일부 HWP 3.0 · HWPML 2.9                                                                     |

#### rhwp 혼자 두면 안 되는 이유 (실측)

**`export-markdown` 이 어떤 문서에서는 본문을 통째로 잃는다.** 표본 18개 중 1개, 표본 52개 중 3개에서 쪽마다 빈 표 한 칸(`|  |`)만 내놓았다. 같은 파일을 `export-text` 로 뽑으면 1,775자가 정상적으로 나오므로 파서가 아니라 **마크다운 내보내기 쪽 결함**이다. 종료 코드는 0 이라 그대로 두면 "성공했는데 내용이 거의 없는 사이드카" 가 생긴다. 조용한 실패는 드러난 실패보다 나쁘다.

그래서 두 리더를 둘 다 돌리고 비교한다.

```text
hwp / hwpx
   |
   +-- 내장 리더 (인프로세스, 1ms)  -> 암호·배포용이면 여기서 끝낸다
   |
   +-- rhwp export-markdown         -> 표가 살아 있는 마크다운
   |
   +-- 내용 글자 수를 견준다 (파이프·하이픈·공백 제외)
          rhwp >= 내장 * 0.6  ->  converter "hwp-markdown"
          그 밖               ->  converter "hwp-text" (내장 리더 본문)
```

내용 있는 파일 68개 중 **64개가 `hwp-markdown`, 4개가 `hwp-text` 폴백**이었다. 폴백한 4개는 전부 rhwp 가 15~~172자만 낸 문서였고 내장 리더는 452~~1,396자를 냈다. 본문을 잃은 문서는 없다.

#### 설치

Dockerfile 의 선택 빌드 인자다. 켜는 곳은 납품 이미지 워크플로 `.github/workflows/chris-deliver-images.yml` 이고 compose 는 손대지 않는다.

```bash
docker build --build-arg OPENCLAW_INSTALL_RHWP=1 .
```

바이너리는 `/opt/rhwp/rhwp` 에 놓인다. 게이트웨이는 `RHWP_BIN` 환경변수 -> `PATH` 의 `rhwp` -> `/opt/rhwp/rhwp` 순으로 찾고, **없으면 내장 리더만으로 계속 동작한다**(LibreOffice 를 다루는 방식과 같다). 이름에 `OPENCLAW_` 접두사를 쓰지 않은 이유는 `SOFFICE_BIN` 과 같다.

#### 버전을 올릴 때

`OPENCLAW_RHWP_VERSION` 과 `OPENCLAW_RHWP_SHA256` 을 함께 바꾸고, 바꾸기 전에 `docker run --rm node:24-bookworm-slim` 에서 새 바이너리가 실제로 실행되는지 확인한다. v0.8.3 부터는 glibc 2.39 빌드라 지금 베이스에서는 실행되지 않는다. 최신판이 꼭 필요해지면 (가) 베이스 이미지를 새 데비안으로 올리거나 (나) 소스에서 빌드하는 단계를 추가해야 하고, 둘 다 이 기능 하나보다 비용이 크다.

#### 아직 안 한 것

- ~~파일 패널 미리보기는 여전히 내장 리더의 텍스트 HTML 이다.~~ **9-6 에서 했다(V 단계).**
- `export-tables --json` 의 격자 좌표는 쓰지 않는다. 지금은 `export-markdown` 의 표만 쓴다.

### 9-6. 미리보기가 표를 표로 그린다 (2026-09-09, V 단계)

S 단계는 rhwp 로 표가 살아 있는 마크다운을 얻어 놓고도 그것을 색인에만 썼고, 파일 패널은 내장 리더의 글자만 보여 주고 있었다. 공유의 한글 문서는 거의 다 양식(발주서·일일보고·결재란)이라 표가 곧 내용이다.

#### 두 리더 중 누가 답하나 - 한 곳으로 모았다

`src/gateway/document-hangul-read.ts`(84줄)를 새로 만들고, 9-5 의 "rhwp 1순위 + 내용 글자 수 60% 하한 + 내장 리더 폴백" 판단을 거기 하나에 두었다. 색인(`src/knowledge/convert.ts`)과 미리보기(`src/gateway/document-extract-html.ts`)가 이 함수를 함께 쓴다. **같은 파일에 두 표면이 다르게 답하지 않는다.**

| 결과 | 색인 `converter` | 미리보기 |
| --- | --- | --- |
| rhwp 마크다운이 하한을 넘었다 | `hwp-markdown` | 표·제목·목록으로 그린다 |
| rhwp 가 없거나 본문을 잃었다 | `hwp-text` | 종전대로 문단만 그린다 |
| 암호·배포용·HWP 3.0 | `failed/<사유>` | "미리보기할 수 없음" |

#### 마크다운을 HTML 로 그리는 것

`src/gateway/document-markdown-html.ts`(224줄). **새 npm 의존성 0개다.**

저장소에 있는 마크다운 라이브러리 둘(`marked`·`markdown-it`)은 **개발 의존성**이라 런타임에 못 쓴다. 그리고 범용 렌더러는 원문에 든 HTML 을 그대로 통과시키는 것이 기본 동작인데, 여기 들어오는 것은 고객 문서 본문이고 미리보기 프레임 안에서 렌더된다. 그래서 **모든 글자를 먼저 이스케이프하고, 태그는 이 파일만 만든다.**

그리는 것은 변환기가 실제로 내는 것만이다.

| 문법 | 결과 |
| --- | --- |
| `## p.N` 등 제목 1~6 | `<h1>`~`<h6>` |
| 파이프 표. 정렬 표시 `:--`·`--:` 와 이스케이프한 파이프를 알아본다 | `<table>`, 가로 스크롤 상자 안 |
| `-`·`*`·`+` 목록, `1.` 목록 | `<ul>`·`<ol>` |
| 빈 줄로 나뉜 문단, 문단 안 줄바꿈 | `<p>`, `<br>` |
| `**굵게**`·`*기울임*`·인라인 코드 | `<strong>`·`<em>`·`<code>` |
| 링크·이미지 문법 | **글자 그대로 남긴다.** 미리보기 프레임이 문서에 적힌 주소를 가져오게 하지 않는다 |

표 스타일은 docx·xlsx 미리보기가 이미 쓰던 `DOCUMENT_STYLE` 을 그대로 쓴다. 한글 양식과 엑셀 표가 같은 제품으로 보인다. 넓은 표는 `.table-scroll` 안에서 가로로 넘치고 본문은 넘치지 않는다.

`escapeHtml` 은 두 렌더러가 함께 쓰므로 `src/gateway/document-html-escape.ts` 로 뺐다.
