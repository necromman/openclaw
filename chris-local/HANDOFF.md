...
- **2026-09-14 AEO/GEO 전문 에이전트(AK)**: 대표 요청으로 검색 답변엔진(네이버 AI 브리핑·AI 탭, ChatGPT, Gemini, Perplexity) 노출 최적화를 전담하는 두 번째 에이전트 `aeo-geo` 를 납품 템플릿에 넣었다. 모델 `anthropic/claude-opus-5`, 사고 수준 medium, 도구는 `coding` 프로필 + `browser`, 워크스페이스 시드에 페르소나 3종과 스킬 4종(페이지 감사·크롤러 접근·JSON-LD 스키마·건강기능식품 금칙 표현)을 함께 담았다. 노출은 부서로 잠근다: `tools.sessions.visibility` 를 `self` 에서 `department` 로 올리고 이 에이전트를 구성원 없는 `dept-ceo` 에 묶어 시스템 관리자와 그 부서에 넣은 사람만 보게 한다. 가시성 전환의 부작용 넷(부서 없는 관리자·임원의 조회 축소, 관리자 우회 해제, 모델 세션 도구 확장, 첨부 경계)은 정본 4-1 절에 정리했고 세 번째는 두 에이전트 모두에 `sessions_*` 도구 4개를 `tools.deny` 로 막아 대응했다. 정본은 [AEO-GEO.md](AEO-GEO.md). 배포는 템플릿 폴더를 NAS 로 옮기고 `deploy.sh --force` 후 `agents department --agent aeo-geo --set ceo` 이며, 운영 실측 11항목은 아직 남아 있다.
