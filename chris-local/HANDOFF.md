...
구현 잔여:
5. TOTP 등록 화면, 한국어 외 19개 로케일(`pnpm ui:i18n:sync` 워크플로)
6. 선재 결함: `chat.history` 일반 가시성 필터 부재, 시작 번들 예산 초과(업스트림 상태)
7. 이메일 열거 타이밍 차(247ms 대 110ms) - IX-Auth 쪽 변경 필요
8. hwp 미리보기(인제스트 파이프라인 범위), xlsx 시트 상호작용, 채팅 첨부 카드 미리보기
9. 브랜딩 잔여: 기본 꺼짐 마스코트 스프라이트·갤러리·앱 아트 18개·네이티브 앱 아이콘
10. claw01 벡터 임베딩 켜기 + RAG 벤치, ai01 → claw01 역방향 통로, 텔레그램 인바운드 파일 실측 1회

설계만 있고 미착수:
11. 인제스트 앱(`knowledge/development/document-ingest-pipeline.md`, 아키텍처 A/B 선택 대기)
