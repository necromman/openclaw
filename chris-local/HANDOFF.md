...
- **Q 단계(2026-09-08 밤, `chris/main` 직접)**: 사용자 지시 4건을 반영했다. 상세는 [DELIVERY-PLAN.md](DELIVERY-PLAN.md) 5절 Q 행.
...
- **다음 세션 첫 할 일**:
  1. **NAS 스택의 남은 세팅.** 실제 공유 폴더 매핑(`OPENCLAW_NAS_ROOT` 가 아직 `nas-sample` 이다, DEPLOY.md 11.4)과 SMTP(DEPLOY.md 3.8). 둘 다 부서 에이전트를 켜거나 메일을 붙일 때 필요하고, NAS 의 `ixauth.env` 를 고치고 `deploy.sh --force` 로 다시 띄우면 된다.
  ...
  3. 부서 에이전트를 다시 켤 때는 DEPLOY.md 3.3-1 의 다섯 단계를 따른다(공유 마운트 -> 템플릿의 에이전트 항목 -> API 키 -> 색인). 지금은 `main` 하나이고 API 키 없이 구독 로그인과 setup-token 으로 두 모델이 다 답한다.
