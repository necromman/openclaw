...
- **2026-09-13 사용자 권한 모달(AD)**: 사용자 상세를 계정 / 역할·부서 / 폴더 권한 모달로 통합했다. 개인 예외·부서 공통·역할 공통을 사용자 화면에서 선택하고 부서 목록에서도 폴더 규칙을 바로 연다. 첫 로그인 전 계정은 기존 로그인과 같은 이메일 프로필 소유자로 연결한다. 저장 중 잠금, 폴더 이동/닫기 변경 버리기 확인, 다른 탭 초안 보존을 포함한다. 배포·검증 정본은 [DELIVERY-PLAN.md](DELIVERY-PLAN.md)의 AD 기록이며 공용 검색의 사용자별 인가는 여전히 별도 과제다.
