...
- **Y 단계(2026-09-09, `chris/main` 직접)**: **OpenAI 를 걷어내고 Claude 단독 구성으로 바꿨다**(사용자 지시: "GPT 는 키 없애고, Anthropic 토큰으로 Claude 를 사용할 수 있게. CLI 는 필요 없다"). 정본은 [DEPLOY.md](DEPLOY.md) 3.4 (사)·(아), 사용자 안내는 [MANUAL.md](MANUAL.md) 10절.
  ...
  - **모델 카탈로그 화면에서 OpenAI 를 감추지 않았다.** 그 화면은 프로바이더별 재고 목록이고, 자격증명 없는 프로바이더는 이미 "인증 없음" 으로 표시되고 스위치를 켤 수 없다(`blocked = !authenticated && !allowAll`). 감추면 관리자가 "무엇을 켤 수 있었는지" 를 볼 길이 없어지고 나중에 되켜는 절차가 보이지 않게 된다. 직원이 보는 채팅 모델 선택기는 `modelPolicy.allow` 가 정하므로 이미 Claude 만 나온다.
