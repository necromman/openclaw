...
- **V 단계(2026-09-09, `chris/main` 직접)**: NAS 폴더 접근권한 **2단계**(적용 지점)와 한글 미리보기 표. 커밋 `da48894d22c` -> `d0ffdef53c1` -> `2dad03a3b16` -> `d0948323dff` -> `cfb183bb685` -> `092893d81fd`. 정본은 [NAS-FOLDER-ACL.md](NAS-FOLDER-ACL.md) 13절, 색인 규칙은 [KNOWLEDGE.md](KNOWLEDGE.md) 8-1, 미리보기는 [FILE-PREVIEW.md](FILE-PREVIEW.md) 9-6, 사용자 안내는 [MANUAL.md](MANUAL.md) 8-1.
  ...
  - **선재 부채: dead export scan**: 포크 export 약 80개(테스트만 소비)가 Knip 에 걸려 `chris-check` 에서 건너뜀(`OPENCLAW_CHECK_CHANGED_SKIP_DEADCODE=1`, 2026-09-10). 정리하려면 export 제거 또는 knip 설정에 소비처 모델링. 대표 파일: `src/gateway/user-activity-audit-query.ts`, `src/state/folder-access-store.ts`, `ui/src/features/ix-auth/ix-auth-role-labels.ts`, `ui/src/brand.ts`, `ui/src/lib/sessions/session-key.ts` 의 `isUiHomeSessionKey`.
