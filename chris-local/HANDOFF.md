...
- **2026-09-14 AGEO 에이전트(AK)**: 대표 요청으로 검색 답변엔진(네이버 AI 브리핑·AI 탭, ChatGPT, Gemini, Perplexity) 노출 최적화를 전담하는 두 번째 에이전트 `ageo`(표시 이름 "AGEO 에이전트", AEO 와 GEO 를 합친 사내 명칭. 같은 날 `aeo-geo`·"AEO/GEO 비서" 에서 이름을 바꿨다)를 납품 템플릿에 넣었다. 모델 `anthropic/claude-opus-5`, 사고 수준 medium, 도구는 `coding` 프로필 + `browser`, 워크스페이스 시드에 페르소나 3종, 회사 컨텍스트 `COMPANY.md`, 스킬 4종(페이지 감사·크롤러 접근·JSON-LD 스키마·`ageo-claims-guard` 금지 표현)을 함께 담았다. 타겟은 라포르몰(`www.cstpillow.com`) 베개·마사지기이고 메디월드몰은 폐쇄형이라 제외한다. 제품이 의료기기 미허가 공산품이라 규제 근거는 의료기기법 제26조제7항 계열이다(정본 3-2, 조사 원문 [AGEO-RESEARCH-2026-09-14.md](AGEO-RESEARCH-2026-09-14.md)). 노출은 부서로 잠근다: `tools.sessions.visibility` 를 `self` 에서 `department` 로 올리고 이 에이전트를 구성원 없는 `dept-ceo` 에 묶어 시스템 관리자와 그 부서에 넣은 사람만 보게 한다. 가시성 전환의 부작용 넷(부서 없는 관리자·임원의 조회 축소, 관리자 우회 해제, 모델 세션 도구 확장, 첨부 경계)은 정본 4-1 절에 정리했고 세 번째는 두 에이전트 모두에 `sessions_*` 도구 4개를 `tools.deny` 로 막아 대응했다. 정본은 [AEO-GEO.md](AEO-GEO.md). 배포는 템플릿 폴더를 NAS 로 옮기고 `deploy.sh --force` 후 `agents department --agent ageo --set ceo` 이며(이름 변경 전에 걸어 둔 옛 id 바인딩은 `--agent aeo-geo --clear` 로 먼저 푼다, 정본 7-1), 운영 실측 11항목은 아직 남아 있다.

- **2026-09-13 대표 계정과 대리 접속(AE)**: 대표 계정을 ACTIVE·SUPERADMIN·이메일 인증 완료 상태로 준비했다. ADMIN·SUPERADMIN의 대리 시작/복귀, 대리 중 관리 제한, 실제 관리자 감사 귀속과 권한 회수 시 대리 세션 종료를 구현했다. 제품 코드 `25a9740e718`, 시험 수정 `59bd5fdf1e1`, 최신 Actions 정적·Node·UI 성공. 14:14 KST 새 이미지와 gateway healthy, 안내 후 217.4초에 원래 주소 HTTP 200 복귀를 확인했다. **사용자 최신 지시에 따라 운영 접속 시험은 페이블이 수행한다.** 비공개 절차는 로컬 `chris-local/infra/local/FABLE-AE-HANDOFF.md`와 private chris-server `analysis/2026-09-13-impersonation/FABLE-HANDOFF.md`에 있다. 페이블 실측 결과(14:17~14:28 KST): SUPERADMIN 대리 시작·대상 신원·관리 API/WS 관리 메서드 거절·실제 chat.send 접수를 확인했고, 대표 비밀번호 설정 링크를 최종 재발급해 폼만 확인했다(미소비). 채팅 final·모바일 캡처·복귀 단계와 임시 ADMIN 회수 시험(C)은 미완이다. 실측 중 NAS 부하(2코어, 부하 평균 5, Synology 이미지 백업 CPU 점유)로 게이트웨이가 SQLite 쓰기에서 108초·51초 멈춰 채팅과 관리자 모달이 지연됐다. 제품 결함이 아니라 운영 자원 문제이며 후속은 DELIVERY-PLAN AE 를 따른다.

- **2026-09-13 사용자 권한 모달(AD)**: 사용자 상세를 계정 / 역할·부서 / 폴더 권한 모달로 통합했다. 개인 예외·부서 공통·역할 공통을 사용자 화면에서 선택한다. 부서도 기본 정보/구성원/폴더 권한/비서 연결의 단일 모달이며, 개인별 폴더 탭은 멤버를 기본 표시한다. 세션 목록은 실제 화면 높이를 측정해 약 1.5화면을 표시한 뒤 더 보기를 제공한다. 초기 진입·로딩 등의 캐릭터는 한국어 상태 표시로 교체하고 펫/도감 진입 코드를 제거했다. 첫 로그인 전 계정은 기존 로그인과 같은 이메일 프로필 소유자로 연결한다. 저장 중 잠금, 폴더 이동/닫기 변경 버리기 확인, 다른 탭 초안 보존을 포함한다. 배포·검증 정본은 [DELIVERY-PLAN.md](DELIVERY-PLAN.md)의 AD 기록이며 공용 검색의 사용자별 인가는 여전히 별도 과제다.

- **2026-09-13 회사 관리자 화면 개선(AC)**: 폴더 및 관련 안내 84개 문구 한글화, 역할 명시 저장, 사용자·부서·폴더의 늦은 이전 응답 차단과 조회 실패 재시도. NAS 앞단 독립 nginx가 배포 중 한국어 503 안내와 준비 완료 후 원래 주소 복귀를 제공한다. NAS compose·maintenance 파일·터널 경로·기존 trustedProxies 값을 함께 반영했다. 변경 이유·후속 권한 과제는 [ENTERPRISE-ACCESS-PLAN.md](ENTERPRISE-ACCESS-PLAN.md) 12절, 정확한 배포·Actions·실측 결과는 [DELIVERY-PLAN.md](DELIVERY-PLAN.md)의 AC 기록. 공용 AI 검색은 아직 사용자별 문서 인가가 아니며, 전체 코어 린트 exit 143과 기존 세션 갱신 실패는 후속 조사 대상이다.

- **2026-09-11 기업용 관리 개선 분석**: [ENTERPRISE-ACCESS-PLAN.md](ENTERPRISE-ACCESS-PLAN.md). NAS SSH와 배포 사이트에 실제 로그인해 계정·그룹·ACL 표본·읽기 전용 마운트, 사용자·부서·폴더·IX-Auth 콘솔을 확인했다. 부서 필터 다음 페이지 오류와 구성원 GET 405를 재현했다. NAS 계정 연결, 권한 정본, 검색 인가, UX 공통화와 단계별 도입 계획을 정리했다. 같은 날 계획 1단계의 코드 항목(P0-1 CSV 부서 연결, P0-3 구성원 GET 구현과 삭제 판정, P1-2 부서 필터 총계·페이지, P1-3 부분 반영 표시, P1-4 알림 유지)을 `chris/main` 에 넣었다(계획 11절, DELIVERY-PLAN 5절 AB 행). 계정/ACL 변경은 하지 않았다. 다음 작업은 배포 후 운영 실측과 실직원 기대 권한표 확정이다. 아래 완료 기록은 각 작성 시점 기준이다.
...
- **Q 단계(2026-09-08 밤, `chris/main` 직접)**: 사용자 지시 4건을 반영했다. 커밋 `b5760968c05`·`d98c98df2e8`·`64dc42c1f2b`, 이미지 빌드 34244196167 성공, NAS 배포 2026-09-09 00:41:24 KST, 운영 실측 4항목 통과(증적 chris-server `analysis/2026-09-07-openclaw-auth/delivery-q-01..04-*.png`). 상세는 [DELIVERY-PLAN.md](DELIVERY-PLAN.md) 5절 Q 행.
  ...
  - **함정: 홈 판정은 표식·패턴 헬퍼로만 한다.** 홈 세션 키가 사람마다 다르므로(`<main>-u<16진수 16자리>`, 옛 공용 홈 `agent:main:main` 도 남아 있다) "내 홈 키와 같은가" 로 비교하면 남의 홈과 옛 공용 홈을 놓친다. 서버는 `src/gateway/home-session-key.ts` 의 `isHomeSessionKey`, 화면은 행의 `home` 표식(없으면 `isUiHomeSessionRow`)을 쓴다. 2026-09-10 에 사이드바가 이것 때문에 목록 전체를 "Main Session" 밑으로 접었다(DELIVERY-PLAN.md 5절 AA 행).
...
- **V 단계(2026-09-09, `chris/main` 직접)**: NAS 폴더 접근권한 **2단계**(적용 지점)와 한글 미리보기 표. 커밋 `da48894d22c` -> `d0ffdef53c1` -> `2dad03a3b16` -> `d0948323dff` -> `cfb183bb685` -> `092893d81fd`. 정본은 [NAS-FOLDER-ACL.md](NAS-FOLDER-ACL.md) 13절, 색인 규칙은 [KNOWLEDGE.md](KNOWLEDGE.md) 8-1, 미리보기는 [FILE-PREVIEW.md](FILE-PREVIEW.md) 9-6, 사용자 안내는 [MANUAL.md](MANUAL.md) 8-1.
  ...
  - **선재 실패**: `chris-check` 의 `plugin boundaries`(만료된 deprecation 창)는 2026-09-10 에 해결했다(만료된 `sdk-untrusted-context-identifier-aliases` 를 업스트림처럼 `removal-pending` 으로 옮겨 `eligibleForRemoval=0`). R 단계부터 빨갛던 포크 테스트 1건(`folder-access-path` 의 `"/"`)은 V 에서 고쳤다.
  - **선재 실패 2건 해소(2026-09-10)**: `chris-check` 의 `plugin-boundary-report` 테스트가 잡아 두었던 `removalPending=8` 단언을 실제 출력 9 로 맞췄고, `check:coercion-helpers` 가 지목하던 포크 지역 변환 헬퍼 10곳(ix-auth 관리 클라이언트·모델 정책·Control UI ix-auth API)을 `@openclaw/normalization-core` 의 `asOptionalRecord`·`readStringValue`·`readStringField`·`asFiniteNumber` 로 바꿔 선언 가드를 통과시켰다.
  - **선재 부채: dead export scan -> 2026-09-10 해소(export 80개 정리, 검사 재활성화)**: Knip 이 지목한 포크 export 를 세 갈래로 정리했다. 어디서도 안 쓰던 19개는 코드째 지우고(브랜드 `BRAND_UPSTREAM_LICENSE_NOTICE`, `KNOWLEDGE_CONVERTIBLE_EXTENSIONS`, 부서 스키마 행 타입 2개, `countFolderRules` 등 + 재수출 8개), 같은 파일 안에서만 쓰던 61개는 `export` 만 뗐다(`ui/src/lib/sessions/session-key.ts` 의 `isUiHomeSessionKey` 포함, 테스트는 `isUiHomeSessionRow` 경유로 바꿨다). 프로덕션 소비처를 Knip 이 못 보는 경우와 초점 테스트만 소비하는 내부 경계는 업스트림 관례대로 `config/knip.config.ts` 의 `ignoreIssues` 에 파일별로 사유와 함께 적었다(28개 파일). `chris-check` 의 `OPENCLAW_CHECK_CHANGED_SKIP_DEADCODE` 는 제거했다.
