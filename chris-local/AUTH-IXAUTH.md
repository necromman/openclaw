...
- **이 화면은 쓰지 않는 프로바이더도 그대로 보여 준다**(Y 단계 판단, 2026-09-09). 납품 구성이 Claude 단독이 된 뒤에도 OpenAI 를 목록에서 감추지 않았다. 자격증명이 없는 프로바이더는 이미 "인증 없음" 으로 표시되고 스위치가 잠기므로(`blocked = !authenticated && !allowAll`) 켤 수 없고, 감추면 관리자가 무엇을 켤 수 있었는지와 되켜는 길을 화면에서 볼 수 없게 된다. 직원이 보는 채팅 모델 선택기는 이 화면이 아니라 `modelPolicy.allow` 가 정하므로 그쪽에는 이미 Claude 만 나온다.
