...
**첫 1분 구멍 (2026-09-13 2차 실측 후속).** 2차 수정 배포 뒤 실측에서 WS 로 일한 세션은 `last_seen_at` 이 밀렸는데 페이지 이동만 반복한 세션은 `created_at` 그대로였다. 추적 결과 `/auth/me` 의 호출 지점(`src/gateway/ix-auth-http.ts` 의 `handleIxAuthSessionProbeRoute`)은 정상이었다. 원인은 활동 기록 쪽의 **문턱 두 개**였다.

| 문턱 | 무엇을 막았나 |
| --- | --- |
| 인메모리 스로틀(세션당 60초) | 의도대로 동작. 통과할 때만 값을 새로 적는다 |
| `nowMs - last_seen_at < 60초` 면 쓰기 생략 | 로그인 직후의 활동을 통째로 버렸다 |

브라우저의 첫 프로브는 로그인 몇 초 뒤에 온다. 그때 `last_seen_at` 은 아직 로그인 시각이라 두 번째 문턱에 걸려 아무것도 쓰지 않고, 스로틀만 찍힌다. 그래서 **로그인 후 첫 1분 안의 활동은 어디에도 남지 않고**, 그 안에서 페이지 이동이 끝난 세션은 `last_seen_at == created_at` 인 채로 남는다. 재현(격리 실행, 22초 간격 프로브): 수정 전에는 t=0·22·44초 프로브가 모두 행을 그대로 두고 t=66초에야 처음 썼다. 수정 후에는 첫 프로브가 바로 쓴다.

지금은 두 번째 문턱을 없앴다. 스로틀을 통과한 활동은 (시계가 뒤로 가지 않는 한) 곧바로 행에 적힌다. 쓰기 빈도는 그대로 세션당 최대 분당 1회다.

- 실측 절차: 로그인하고 페이지를 한 번 새로고침한 뒤 그 세션 행을 본다. `last_seen_at > created_at` 이면 반영된 이미지다. 1분을 기다릴 필요가 없다.
- 회귀 잠금: `src/gateway/ix-auth-http-session-activity.test.ts`(`/auth/me` 성공 → 행이 밀린다·같은 스로틀 창의 연속 로드는 한 번만 쓴다·쿠키 없는 프로브는 행을 건드리지 않는다), `src/auth/ix-auth/ix-auth-session-activity.test.ts` 의 "첫 활동" 항목.
