...
- **가장 큰 조절 손잡이는 사고 수준이다(결정 대기).** 작성칸의 "사고 수준" 은 지금 **High** 이고, 이 값은 우리 설정이 아니라 모델 계약에서 온다(`defaultsClaudeAdaptiveThinking` 이 Sonnet 5·Opus 5 에서 참이라 값을 비워 두면 적응형/High 가 된다). 같은 세션에서 같은 형식의 질문을 사고 수준만 바꿔 재면 이렇게 차이가 난다.

  | 사고 수준 | 실측 |
  | --- | --- |
  | High(현재 기본) | 14.5초 / 17.9초 / 18.3초 / 33.8초 |
  | Minimal | 6.5초 / 6.9초 |

  낮추는 방법은 `agents.defaults.thinkingDefault` 한 줄이다(`"off"`·`"minimal"`·`"low"`·`"medium"`·`"high"`·`"adaptive"` 등). **답의 깊이가 함께 얕아지므로 사용자 결정 사항으로 남겨 두었다.** 사람마다 필요할 때만 낮추는 길은 이미 있다 - 작성칸의 사고 수준을 그 대화에서만 내리면 된다.
...
2026-09-13 AC부터 NAS 앞단에 독립 nginx를 둔다. 파일·접속 경로·설치와 복구·실측 절차는 [nas/maintenance/README.md](nas/maintenance/README.md)가 정본이다. 저장소 푸시만으로 이 호스트 파일과 터널 origin은 바뀌지 않으므로 최초 설치를 별도로 수행한다. 정상 이미지 교체는 기존 `deploy.sh`의 `up -d`를 유지한다.
