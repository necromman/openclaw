...
NAS의 게이트웨이 헬스체크는 이 배치에서 고정한 `18789/healthz`를 직접 조회한다. 범용 설정 로더를 거치던 점검은 실측 27초로 10초 제한을 초과했지만, 같은 정상 서버의 직접 조회는 247ms였다. 안내 화면의 자동 복귀는 이 생존 점검과 별도로 `/readyz`의 준비 완료를 확인한다. 읽기 전용 nginx의 모든 임시 경로는 `/tmp` 아래에 두고, 화면 분기와 오류 안내가 연속 처리되도록 서버 범위의 `recursive_error_pages`를 켠다.
