...
2026-09-13 대리 접속: 대상 계정의 활동 행에는 기존 `detail` 객체 안에 `impersonator.subject`와 `impersonator.email`을 함께 기록한다. 값은 검증된 토큰의 `act`에서 연결·질문·도구 실행으로 전달한다. 긴 상세 내용이 제한을 넘어 생략돼도 대리 관리자 정보는 남긴다. 대리 시작·종료는 실제 관리자에게 귀속된 `admin_action`이며, 테이블·열·스키마 버전은 바꾸지 않는다.
...
**읽는 곳이 둘이다.** 먼저 RPC 를 부르고, RPC 가 _답하지 못했을 때만_ 이 호스트의 상태 SQLite 를 직접 읽는다
...
- **답한 거절은 폴백하지 않는다.** 핸들러가 `FORBIDDEN`·`INVALID_REQUEST` 로 답했다면 그것은 결정이다. 그 결정을
  우회해 로컬을 읽으면 인가 판정이 인가 우회가 된다. 핸들러가 돌기 _전에_ 끝난 실패(소켓 종료·타임아웃·자격증명
