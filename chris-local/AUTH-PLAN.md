...
**감독 확정 기본값 5개**(전부 "가정 - 사용자 미확정")는 DEPLOY.md 0절에 표로 있다(B 단계에서 부서 항목이 6번째로 추가됐다): 콘솔 미노출·BFF 중계, 역할 4단계 시드, PostgreSQL 16, `ui:i18n:sync` 로 한국어, 포트 18800.
...
### B 단계 - 부서 강제 department-access (2026-09-07 완료)

| 항목                  | 결과                                                                                           | 정본                                             |
| --------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| 부서 모델·규칙 확정   | 완료. 경계는 에이전트에 긋는다(1.1 (D) 채택안 그대로). 역할과 부서는 직교                      | [AUTH-DEPARTMENTS.md](AUTH-DEPARTMENTS.md) 2·3절 |
| 스키마 3표            | 완료. feature-local DDL, 스키마 버전 15 유지, first-use lazy ensure. 승인 근거를 문서에 남겼다 | AUTH-DEPARTMENTS.md 6절                          |
| 로그인 시 동기화·회수 | 완료. 클레임에서 사라진 소속은 다음 로그인에 삭제되고 access revision 을 올린다                | `src/auth/ix-auth/ix-auth-departments.ts`        |
| 강제 지점             | 완료. 목록·직접 열람·전사·이벤트·에이전트 선택·생성 게이트·외부 카탈로그·HTTP 표면             | AUTH-DEPARTMENTS.md 7절                          |
| 3.2 훅 10곳           | 8곳 구현, 2곳은 불필요·범위 밖으로 판정하고 근거를 남겼다                                      | AUTH-DEPARTMENTS.md 7절 대비표                   |
| 테스트                | 완료. 부서 매트릭스 21건(4역할 x 2부서 x 3 대상군) + 기존 모드 회귀                            | `src/gateway/department-access.test.ts`          |
| 라이브 검증           | 완료. compose 스택 18800, 사용자 6명·에이전트 3개. 교차 접근 200/404 대칭 실측                 | AUTH-DEPARTMENTS.md 11절                         |

**감독 확정 기본값 3개**(전부 "가정 - 사용자 미확정")는 AUTH-DEPARTMENTS.md 5절에 있다:
`dept-` 접두사, 에이전트 1개당 부서 1개, 미바인딩 에이전트는 공용.

**실측으로 잡은 결함 4건**

| #   | 증상                                                           | 원인                                                                                                                      |
| --- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| 1   | 남의 부서 전사를 HTTP 로 200 과 함께 받았다                    | HTTP 합성 클라이언트에 검증된 부서가 안 실려 "부서 없는 호출자" 로 인가됐다                                               |
| 2   | Gateway 클라이언트가 없는 HTTP 표면이 생성 게이트를 건너뛰었다 | `openai-http`·`openresponses-http` 가 client 대신 profileId 를 넘긴다                                                     |
| 3   | 기본 진입 화면이 "was not found" 로 죽었다                     | 미바인딩 에이전트를 생성자 전용으로 둔 첫 설계. 규칙을 바꿔 되돌렸다                                                      |
| 4   | superadmin 이 관리 API 403 으로 잠겼다                         | `PUT /admin/users/{id}/roles` 본문 필드가 `roles` 인데 `codes` 로 보냈고, IX-Auth 가 400 대신 조용히 기본 역할로 되돌린다 |

**보류**

| 항목                                       | 사유                                                                                    |
| ------------------------------------------ | --------------------------------------------------------------------------------------- |
| `chat.history` 의 일반 가시성 필터 부재    | 선재 결함. 기존 모드의 동작을 바꾸는 변경이라 별도 심사 필요. 이번엔 부서 펜스만 넣었다 |
| 같은 부서 내 에이전트 간 모델 도구 열람    | 도구 축은 보수적으로 에이전트 경계 판정. 후속                                           |
| 구버전 open/use -> 신버전 reopen 왕복 실측 | 미실시. 신규 표만 추가하는 경로라 문서가 허용하지만 실측은 남았다                       |
| 로컬 CLI `agents list`·`sessions`          | 게이트웨이를 안 탄다. 호스트 셸은 경계 밖이라는 판정                                    |

### C 단계 - 초대장·가입 플로우 (2026-09-07 완료)

| 항목                | 결과                                                                                                | 정본                                 |
| ------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------ |
| IX-Auth 계약 확인   | 계약에 이미 signup·invite·reset·verify 가 전부 있었다. `ix-auth/` 는 한 줄도 고치지 않았다          | [AUTH-SIGNUP.md](AUTH-SIGNUP.md) 6절 |
| 게이트웨이 BFF 표면 | 익명 5개 + 메일 훅 1개 + 관리자 3개. 기존 `/auth/*` stage 재사용, 새 병렬 경로 없음                 | AUTH-SIGNUP.md 6절                   |
| 화면                | 가입·비밀번호 찾기·재설정·이메일 인증·초대 수락 + 관리자 초대·승인 블록                             | AUTH-SIGNUP.md 5절                   |
| 이메일 열거 방지    | 존재/비존재 응답을 상수 하나로 직렬화. 테스트가 본문 문자열을 그대로 비교한다                       | AUTH-SIGNUP.md 4절                   |
| 부서 접점           | 초대 발급·가입 승인 시점에 IX-Auth 그룹 가입까지 끝난다. 첫 로그인부터 부서가 붙는다                | AUTH-SIGNUP.md 7절                   |
| 테스트              | 신규 74건(초대 링크 보관 15 · 익명 표면 28 · 관리자 표면 31) + UI 27건, 기존 ix-auth 23건 회귀 통과 | `src/gateway/ix-auth-*.test.ts`      |
| 라이브 검증         | 두 경로 실측                                                                                        | AUTH-SIGNUP.md 11절                  |

**감독 확정 기본값 8개**(전부 "가정 - 사용자 미확정")는 AUTH-SIGNUP.md 12절에 있다.

**보류**

| 항목                           | 사유                                                                   |
| ------------------------------ | ---------------------------------------------------------------------- |
| 고객 SMTP 자격증명·발신 도메인 | 사용자 결정 사항. `.env` 항목과 mailpit 프로파일만 준비했다            |
| 약관 동의 화면                 | 게시된 약관이 없으면 `GET /auth/terms` 가 빈 목록이라 동의 단계가 없다 |
