...
3. **IX-Auth 콘솔 SSO**: 시스템 관리자가 `/admin/identity/` 로 들어가면 재로그인 없이 진입. 게이트웨이가 IX-Auth 신원 교환으로 같은 사람의 콘솔 토큰을 발급해 심는다. admin 이하는 기존대로 링크 미노출·중계 차단.
...
**2026-09-08 추가**: 5. 부서 에이전트(NAS 폴더 읽기 전용) 모델 인증. ChatGPT 구독(codex)으로는 폴더 경계를 지키며 읽을 수 없어 OpenAI 또는 Anthropic **API 키**가 필요하다(main 에이전트는 구독 codex 유지 가능). 키 종류·비용 부담 주체를 정해야 G 의 폴더 접근 기능이 실제로 동작한다.
